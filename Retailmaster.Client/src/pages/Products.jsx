...
function Toast({ toast }) {
  ...
  return (
    <motion.div initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 20, opacity: 0 }}
      className={`fixed top-5 right-5 z-[9999] alert text-sm shadow-xl w-auto max-w-[90vw]
    ...
    <div className="max-w-[1100px] pb-20 lg:pb-0">
      ...
      <motion.div variants={fadeUp} initial="hidden" animate="visible"
        ...
        <div className="min-w-0">
          <h1 className="text-xl sm:text-2xl font-bold text-base-content tracking-tight">Products</h1>
          <p className="text-xs sm:text-sm text-base-content/50 mt-0.5">
            {products.length} products · Manage branch stock separately
          </p>
        </div>
      ...
      {/* Search */}
      <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={1} className="mb-4 sm:mb-5">
        <label className="input input-bordered flex items-center gap-2 w-full">
          ...
          {search && (
            <button onClick={() => setSearch('')}
              className="text-base-content/30 hover:text-base-content/60 bg-transparent border-0 cursor-pointer">
      ...
      {loading ? (
        <div className="flex flex-col gap-3">
          {[1,2,3].map(i => <div key={i} className="skeleton h-16 sm:h-20 w-full rounded-2xl" />)}
        </div>
      ) : products.length === 0 ? (
        <div className="card bg-base-100 border border-base-300 py-16 sm:py-20 text-center">
          <i className="bi bi-box-seam text-4xl sm:text-5xl text-base-content/20 block mb-3" />
          <p className="text-base-content/50 font-medium mb-4 text-sm sm:text-base">
            No products in catalogue yet
          </p>
          <button onClick={() => setCreateModal(true)}
            className="btn btn-primary btn-sm gap-2 mx-auto">
          ...
          {products.map((product, i) => (
            <motion.div key={product.id} variants={listItem} initial="hidden" animate="visible" custom={i}
              className="card bg-base-100 border border-base-300 shadow-sm overflow-hidden
                hover:border-base-content/20 transition-all">

              {/* Product row — tappable accordion header */}
              <div
                ...
                {/* Icon */}
                <div className="w-9 h-9 sm:w-10 sm:h-10 bg-primary/10 border border-primary/20
                  rounded-xl flex items-center justify-center flex-shrink-0">
                  <i className="bi bi-box-seam-fill text-primary text-sm sm:text-base" />
                </div>

                {/* Name + prices */}
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
                    <span className="font-bold text-base-content text-sm leading-tight truncate">
                      {product.name}
                    </span>
                    {product.barcode && (
                      <span className="badge badge-ghost badge-sm font-mono hidden sm:inline-flex">
                        {product.barcode}
                      </span>
                    )}
                  </div>
                  <div className="flex items-center gap-2 sm:gap-3 mt-0.5 flex-wrap">
                    <span className="text-[10px] sm:text-xs text-base-content/50">
                      Buy: <strong className="text-base-content/70">{fmt(product.purchasePrice)}</strong>
                    </span>
                    <span className="text-[10px] sm:text-xs text-base-content/50">
                      Sell: <strong className="text-primary">{fmt(product.sellingPrice)}</strong>
                    </span>
                    <span className="text-[10px] sm:text-xs text-success hidden sm:inline">
                      Profit: {fmt(product.sellingPrice - product.purchasePrice)}
                    </span>
                  </div>
                </div>

                {/* Branch stock badges — hidden on mobile */}
                <div className="hidden md:flex items-center gap-1.5 flex-wrap justify-end flex-shrink-0 max-w-[260px]">
                  {product.branchStocks?.length === 0 ? (
                    ...
                    <span key={bs.branchId}
                      className={`badge badge-sm
                        ${bs.isLowStock ? 'badge-error badge-outline' : 'badge-success badge-outline'}`}>
                      {bs.branchName}: {bs.stockQty}
                    </span>
                  ))}
                </div>

                {/* Stock count summary on mobile */}
                <div className="flex md:hidden items-center gap-1">
                  {product.branchStocks?.length === 0 ? (
                    ...
                    <span className="badge badge-success badge-sm badge-outline">
                      {product.branchStocks?.length}b
                    </span>
                  )}
                </div>

                {/* Action buttons */}
                <div className="flex gap-1 sm:gap-1.5 flex-shrink-0" onClick={e => e.stopPropagation()}>
                ...
                <i className={`bi bi-chevron-down text-base-content/30 text-xs
                  transition-transform flex-shrink-0
                  ${expanded === product.id ? 'rotate-180' : ''}`} />
              </div>

              {/* Expanded branch stock panel */}
              <AnimatePresence>
                {expanded === product.id && (
                  <motion.div
                    ...
                    className="border-t border-base-300 bg-base-200/50 px-3 sm:px-5 py-4 overflow-hidden">
                    ...
                    <div className="flex items-center justify-between mb-3">
                      ...
                      {unassignedBranches(product).length > 0 && (
                        <button
                          onClick={() => setStockModal({ product, branch: null, mode: 'assign' })}
                          className="btn btn-xs btn-primary btn-outline gap-1.5">
                          <i className="bi bi-plus-lg" />
                          <span className="hidden sm:inline">Assign to Branch</span>
                    ...
                    {product.branchStocks?.length === 0 ? (
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                        {product.branchStocks?.map(bs => (
                          <div key={bs.branchId}
                            className={`flex items-center gap-3 bg-base-100 border rounded-xl
                              px-3 sm:px-3.5 py-3
                              ${bs.isLowStock ? 'border-error/30' : 'border-base-300'}`}>
                            <div className={`w-8 h-8 rounded-lg flex items-center justify-center
                              flex-shrink-0
                              ${bs.isLowStock ? 'bg-error/10' : 'bg-success/10'}`}>
                              <i className={`bi bi-building-fill text-xs
                                ${bs.isLowStock ? 'text-error' : 'text-success'}`} />
                            </div>
                            <div className="flex-1 min-w-0">
                              ...
                              <div className={`text-sm font-black font-mono
                                ${bs.isLowStock ? 'text-error' : 'text-base-content'}`}>
                                {bs.stockQty} units
                                {bs.isLowStock && (
                                  <span className="text-[10px] font-semibold ml-1 text-error/70">Low</span>
                                )}
                              </div>
                              <div className="text-[10px] text-base-content/40">
                                Min: {bs.lowStockThreshold}
                              </div>
                            </div>
...
function CashierProducts({ branchId, branchName }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await api.get(
        `/products?branchId=${branchId}${search ? `&search=${encodeURIComponent(search)}` : ''}`
      )
      setProducts(r.data)
    } catch {}
    finally { setLoading(false) }
  ...
  return (
    <div className="max-w-[900px] pb-20 lg:pb-0">
      {/* Header */}
      <motion.div variants={fadeUp} initial="hidden" animate="visible" className="mb-5 sm:mb-6">
        <h1 className="text-xl sm:text-2xl font-bold text-base-content tracking-tight">Products</h1>
        <p className="text-xs sm:text-sm text-base-content/50 mt-0.5 flex items-center gap-1.5 flex-wrap">
          <i className="bi bi-building text-primary" />{branchName}
          <span className="text-base-content/20">·</span>
          {products.length} products
          {low.length > 0 && (
            <span className="text-error font-semibold">· {low.length} low stock</span>
          )}
        </p>
      </motion.div>
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-3 mb-4 sm:mb-5">
        {[
          ...
          <motion.div key={s.label} variants={fadeUp} initial="hidden" animate="visible" custom={i}
            ...
            <div className="card-body p-3 sm:p-3.5 flex-row items-center gap-2 sm:gap-3">
              <div className={`w-8 h-8 sm:w-9 sm:h-9 bg-${s.color}/10 rounded-xl
                flex items-center justify-center flex-shrink-0`}>
                <i className={`bi ${s.icon} text-${s.color} text-sm`} />
              </div>
              <div className="min-w-0">
                <div className="text-[10px] text-base-content/50 font-semibold truncate">{s.label}</div>
                <div className={`text-lg sm:text-xl font-black font-mono text-${s.color}`}>{s.value}</div>
              </div>
            </div>
          </motion.div>
        ))}
      ...
      {/* Search */}
      <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={3} className="mb-4">
        <label className="input input-bordered flex items-center gap-2 w-full">
          <i className="bi bi-search text-base-content/30" />
          <input value={search} onChange={e => setSearch(e.target.value)}
      ...
      {loading ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 sm:gap-3">
          {[1,2,3,4,5,6].map(i => <div key={i} className="skeleton h-24 sm:h-28 w-full rounded-2xl" />)}
        </div>
      ) : products.length === 0 ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 sm:gap-3">
          {products.map((p, i) => (
            <motion.div key={p.id} variants={listItem} initial="hidden" animate="visible" custom={i}
              className={`card bg-base-100 border-2 p-3 sm:p-4 flex flex-col gap-2
              ...
              <div className="flex items-start justify-between gap-1">
                <div className="text-xs sm:text-sm font-bold text-base-content leading-tight line-clamp-2">
                  {p.name}
                </div>
                {p.barcode && (
                  <span className="badge badge-ghost badge-xs font-mono flex-shrink-0 hidden sm:inline-flex">
                    {p.barcode}
                  </span>
                )}
              </div>
              ...
              <div className={`flex items-center gap-1.5 text-[10px] sm:text-xs font-semibold mt-auto
                ${p.stockQty === 0
                  ? 'text-base-content/40'
                  : p.isLowStock
                    ? 'text-error'
      ...
      <div
        ...
        <div className="card-body p-4 sm:p-6">
          <div className="flex items-start justify-between mb-1">
            <div>
              <h3 className="font-bold text-base-content text-sm sm:text-base">{title}</h3>
              {subtitle && (
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-3 sm:gap-4 mt-3">
            ...
            <div className="form-control">
              ...
              <input value={form.barcode} onChange={set('barcode')}
                placeholder="Optional" className="input input-bordered w-full" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="form-control">
                ...
                <input type="number" step="0.01" min="0" value={form.purchasePrice}
                  onChange={set('purchasePrice')} required placeholder="0.00"
                  className="input input-bordered w-full" />
              </div>
              <div className="form-control">
                <label className="label pb-1.5">
            ...
            {margin && (
              <div className="alert alert-success text-xs py-2">
                ...
                Margin: {margin}% · Profit: {fmt(form.sellingPrice - form.purchasePrice)}
              </div>
            )}
  ...
  return (
    <motion.div variants={scaleIn} initial="hidden" animate="visible" exit="exit"
      className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-end sm:items-center
        justify-center z-[1000] p-0 sm:p-5"
      onClick={onClose}>
      <div
        className="card bg-base-100 w-full sm:max-w-[420px] shadow-2xl
        ...
        <div className="card-body p-4 sm:p-6">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-3 sm:gap-4 mt-3">
            ...
            {mode === 'adjust' && (
              <div className="flex items-center gap-2.5 bg-primary/10 border border-primary/20
                rounded-xl px-4 py-3">
                <i className="bi bi-building-fill text-primary" />
                <span className="text-sm font-bold text-primary">{bs.branchName}</span>
                <span className="ml-auto text-xs text-primary/60">Current: {bs.stockQty}</span>
              </div>
            )}
            <div className="grid grid-cols-2 gap-3">
              <div className="form-control">
    ...
    <motion.div variants={scaleIn} initial="hidden" animate="visible" exit="exit"
      ...
      <div
        ...
        <div className="card-body p-5 sm:p-6 text-center">
          ...
          <h3 className="font-bold text-base-content text-sm sm:text-base mb-1">{title}</h3>
