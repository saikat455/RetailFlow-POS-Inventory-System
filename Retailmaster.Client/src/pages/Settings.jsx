  ...
  return (
    <motion.div variants={scaleIn} initial="hidden" animate="visible"
      ...
      <div className="flex items-center gap-3 px-6 py-4 border-b border-base-300">
        ...
        <div>
          <div className="font-bold text-base-content text-sm">{title}</div>
...
function ProfileSection({ showToast, user, onNameUpdate }) {
  ...
  return (
    <SectionCard title="My Profile" subtitle="Update your name and password" icon="bi-person-fill">
      {/* Avatar strip */}
      <div className="flex items-center gap-4 p-4 bg-gradient-to-r from-primary/10 to-secondary/10 border border-primary/20 rounded-xl mb-5">
        <div className="w-14 h-14 bg-primary rounded-2xl flex items-center justify-center text-2xl font-black text-white flex-shrink-0 shadow-lg shadow-primary/20">
          {profile?.name?.charAt(0).toUpperCase() || '?'}
        </div>
        <div className="flex-1 min-w-0">
          <div className="font-bold text-base-content text-base">{profile?.name}</div>
          <div className="text-sm text-base-content/60">{profile?.email}</div>
          <div className="flex items-center gap-2 mt-1">
            <span className={`badge badge-sm ${profile?.role === 'Admin' ? 'badge-warning' : 'badge-primary'}`}>
              {profile?.role}
            </span>
            {profile?.branchName && (
              <span className="text-[10px] font-semibold text-base-content/40 flex items-center gap-1">
                <i className="bi bi-building text-base-content/30" />{profile.branchName}
              </span>
            )}
          </div>
        </div>
        <div className="ml-auto text-right flex-shrink-0">
          <div className="text-[10px] text-base-content/40">Member since</div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        <div className="form-control">
          <label className="label pb-1.5"><span className="label-text text-[11px] font-bold uppercase tracking-wide">Full Name *</span></label>
          <label className="input input-bordered flex items-center gap-2">
            <i className="bi bi-person text-base-content/30" />
            <input value={form.name} onChange={e => setForm(f => ({...f, name: e.target.value}))}
              required placeholder="Your full name" className="grow text-sm" />
          </label>
        </div>

        <div className="form-control">
          <label className="label pb-1.5"><span className="label-text text-[11px] font-bold uppercase tracking-wide">Email Address</span></label>
          <label className="input input-bordered input-disabled flex items-center gap-2 opacity-60">
            ...
            <input value={profile?.email || ''} readOnly className="grow text-sm cursor-not-allowed" />
          </label>
          <label className="label pt-1"><span className="label-text-alt text-base-content/40">Email cannot be changed</span></label>
        </div>
        ...
        <label className="label cursor-pointer justify-start gap-3 p-3 bg-base-200 rounded-xl border border-base-300 hover:border-primary/30 transition-colors">
          <input type="checkbox" checked={changingPass} onChange={e => setChangingPass(e.target.checked)}
            className="toggle toggle-primary toggle-sm" />
          <div>
            <span className="label-text font-semibold block">Change password</span>
            <span className="label-text-alt text-base-content/50">Leave off to keep current password</span>
          </div>
        </label>
        ...
        <AnimatePresence>
          {changingPass && (
            <motion.div variants={fadeUp} initial="hidden" animate="visible" exit={{ opacity: 0, height: 0 }}
              className="flex flex-col gap-3 p-4 bg-base-200 rounded-xl border border-base-300">
              {[
                ...
                <div key={field.key} className="form-control">
                  <label className="label pb-1.5"><span className="label-text text-[11px] font-bold uppercase tracking-wide">{field.label}</span></label>
                  <label className={`input flex items-center gap-2
                    ...
                    <input type={showPass ? 'text' : 'password'}
                      ...
                      placeholder={field.ph} className="grow text-sm" />
                    {field.showToggle && (
        ...
        <button type="submit" disabled={loading} className="btn btn-primary w-full gap-2">
          {loading ? <><span className="loading loading-spinner loading-sm" /> Saving…</> : <><i className="bi bi-check-lg" /> Save Profile</>}
        </button>
      </form>
    </SectionCard>
  )
      ...
      {company && (
        <div className="grid grid-cols-3 gap-3 mb-5">
          {[
            ...
            { label: 'Since',    value: new Date(company.createdAt).getFullYear(), icon: 'bi-calendar', color: 'text-secondary' },
          ].map(s => (
            <div key={s.label} className="bg-base-200 rounded-xl p-3 text-center">
              <i className={`bi ${s.icon} text-base ${s.color} block mb-1`} />
              <div className="text-lg font-black text-base-content">{s.value}</div>
              <div className="text-[10px] text-base-content/40 font-semibold uppercase tracking-wide">{s.label}</div>
            </div>
          ))}
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        {[
          ...
          <div key={f.key} className="form-control">
            <label className="label pb-1.5"><span className="label-text text-[11px] font-bold uppercase tracking-wide">{f.label}</span></label>
            <label className="input input-bordered flex items-center gap-2">
              <i className={`bi ${f.icon} text-base-content/30`} />
              <input value={form[f.key]} onChange={e => setForm(p => ({...p, [f.key]: e.target.value}))}
                required={f.req} placeholder={f.ph} className="grow text-sm" />
            </label>
          </div>
        ))}
        <button type="submit" disabled={loading} className="btn btn-primary w-full gap-2">
          {loading ? <><span className="loading loading-spinner loading-sm" /> Saving…</> : <><i className="bi bi-check-lg" /> Save Company</>}
        </button>
      </form>

      {company?.inviteCode && (
        <div className="mt-5 p-4 bg-warning/10 border border-warning/30 rounded-xl">
          <div className="text-[11px] font-bold text-warning uppercase tracking-wide mb-2 flex items-center gap-1.5">
            <i className="bi bi-info-circle-fill" /> Company Invite Code
          </div>
          <p className="text-xs text-base-content/60 mb-3">
            Share <strong>branch invite codes</strong> (from the Branches page) with cashiers to lock them to a specific branch.
          </p>
          <div className="flex items-center gap-2">
            <span className="font-mono font-bold text-base tracking-widest text-base-content bg-warning/20 px-3 py-1.5 rounded-lg">
              {company.inviteCode}
            </span>
            <button onClick={copyCode}
              className={`btn btn-sm gap-1.5 ${copied ? 'btn-success' : 'btn-warning btn-outline'}`}>
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-4">
          {[{ label: 'Admins', list: admins }, { label: 'Cashiers', list: cashiers }].map(({ label, list }) =>
            list.length > 0 && (
              <div key={label}>
                <div className="text-[10px] font-bold uppercase tracking-widest text-base-content/40 mb-2">{label}</div>
                <div className="flex flex-col gap-2">
                  {list.map((u, i) => (
                    <motion.div key={u.id} variants={listItem} initial="hidden" animate="visible" custom={i}
                      className="flex items-center gap-3 p-3 bg-base-200 border border-base-300 rounded-xl hover:border-base-content/20 transition-colors">
                      <div className={`w-9 h-9 rounded-xl flex items-center justify-center font-black text-sm flex-shrink-0
                        ${u.role === 'Admin' ? 'bg-warning text-white' : 'bg-primary text-white'}`}>
                        {u.name.charAt(0).toUpperCase()}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="text-sm font-bold text-base-content truncate">{u.name}</div>
                        <div className="text-xs text-base-content/50 truncate flex items-center gap-1.5">
                          <span>{u.email}</span>
                          {u.branchName && (
                            <><span className="text-base-content/20">·</span>
                            <span className="flex items-center gap-1">
                              <i className="bi bi-building text-[10px] text-base-content/30" />{u.branchName}
                            </span></>
                          )}
                        </div>
                      </div>
                      <div className="flex items-center gap-2 flex-shrink-0">
                        <span className={`badge badge-sm ${u.role === 'Admin' ? 'badge-warning' : 'badge-primary'}`}>{u.role}</span>
                        {u.role !== 'Admin' && (
                          <button onClick={() => setDeleteTarget(u)} className="btn btn-xs btn-ghost btn-square text-error hover:bg-error/10">
      ...
      <AnimatePresence>
        {deleteTarget && (
          <motion.div variants={scaleIn} initial="hidden" animate="visible" exit="exit"
            ...
            <div className="card bg-base-100 w-full max-w-[360px] shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
              <div className="card-body p-6 text-center">
                <div className="w-14 h-14 bg-error/10 border-2 border-error/20 rounded-2xl flex items-center justify-center mx-auto mb-4">
                  <i className="bi bi-person-x-fill text-error text-2xl" />
                </div>
                <h3 className="font-bold text-base-content text-base mb-1">Remove User?</h3>
                <p className="text-sm text-base-content/50 mb-5">
                  <strong className="text-base-content">{deleteTarget.name}</strong> will lose access to the system.
                </p>
  ...
  return (
    <div className="max-w-[720px]">
      ...
      <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={1}
        className="flex gap-1 bg-base-200 rounded-xl p-1 mb-6 w-fit">
        {tabs.map(tab => (
          <button key={tab.key} onClick={() => setActiveTab(tab.key)}
            className={`btn btn-sm gap-2 ${activeTab === tab.key ? 'btn-primary shadow-sm' : 'btn-ghost text-base-content/60'}`}>
      ...
      <AnimatePresence mode="wait">
        {activeTab === 'profile' && (
          <motion.div key="profile" variants={fadeUp} initial="hidden" animate="visible" exit={{ opacity: 0 }}>
            <ProfileSection showToast={showToast} user={user} onNameUpdate={handleNameUpdate} />
          </motion.div>
        )}
        {activeTab === 'company' && user?.isAdmin && (
          <motion.div key="company" variants={fadeUp} initial="hidden" animate="visible" exit={{ opacity: 0 }}>
            <CompanySection showToast={showToast} />
          </motion.div>
        )}
        {activeTab === 'users' && user?.isAdmin && (
          <motion.div key="users" variants={fadeUp} initial="hidden" animate="visible" exit={{ opacity: 0 }}>
