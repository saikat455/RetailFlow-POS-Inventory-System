...
export default function POS() {
  ...
  const updateQty  = (id, qty) => {
    ...
    if (!item) return
    const n = Math.max(1, Math.min(parseInt(qty) || 1, item.maxQty))
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-4 py-3.5 border-b border-base-300 flex-shrink-0">
        <h2 className="text-sm font-bold text-base-content flex items-center gap-2">
          ...
          {cartCount > 0 && <span className="badge badge-primary badge-sm">{cartCount}</span>}
        </h2>
        {cart.length > 0 && (
      ...
      {cart.length === 0 ? (
        <div className="flex-1 flex flex-col items-center justify-center gap-2 text-base-content/20">
          <i className="bi bi-cart text-5xl" />
              ...
              <motion.div key={item.productId}
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                  ...
                  <input type="number" value={item.quantity} min={1} max={item.maxQty}
                    ...
                    className="w-8 text-center border border-base-300 rounded-md text-xs font-bold
                      font-mono py-0.5 outline-none focus:border-primary bg-base-100" />
                  <button onClick={() => updateQty(item.productId, item.quantity + 1)}
                    className="btn btn-xs btn-ghost border border-base-300 w-6 h-6 min-h-0 p-0 font-bold">+</button>
                </div>
                <div className="text-xs font-bold font-mono text-primary min-w-[52px] text-right">
      ...
      <div className="border-t border-base-300 px-4 py-3 flex flex-col gap-2 flex-shrink-0">
        ...
        <div className="flex items-center justify-between text-sm">
          ...
          <input type="number" value={discount} min={0} max={totalAmount}
            ...
            className="w-20 text-right border border-base-300 rounded-lg px-2 py-1
              text-xs font-mono outline-none focus:border-primary bg-base-100" />
        </div>
      ...
      <button onClick={handleCheckout} disabled={!cart.length || loading}
        className="btn btn-primary mx-3 mb-3 gap-2">
        {loading
          ? <><span className="loading loading-spinner loading-sm" /> Processing…</>
          : <><i className="bi bi-bag-check-fill" /> Complete Sale — {fmt(finalAmount)}</>}
      </button>
    </div>
  )
  ...
  return (
    <div className="pb-20 lg:pb-0">
      ...
      {user?.isAdmin && branches.length > 0 && (
        <div className="flex items-center gap-2 bg-base-100 rounded-xl border border-base-300
          px-3 py-2 mb-3 flex-shrink-0">
          <i className="bi bi-building-fill text-primary text-sm" />
          <select value={branchId} onChange={e => setBranchId(parseInt(e.target.value))}
            className="flex-1 text-sm font-semibold text-base-content border-0 outline-none
      ...
      {!user?.isAdmin && (
        <div className="flex items-center gap-2 bg-primary/10 border border-primary/20
          rounded-xl px-3 py-2 mb-3 flex-shrink-0">
          <i className="bi bi-building-fill text-primary text-sm" />
      ...
      <div className="flex gap-4 h-[calc(100vh-180px)] lg:h-[calc(100vh-120px)]">
        ...
        <div className="flex-1 flex flex-col gap-3 min-w-0 overflow-hidden">
          <label className="input input-bordered border-2 focus-within:border-primary flex items-center gap-2">
            ...
            {search && (
              <button onClick={() => setSearch('')}
                className="text-base-content/30 bg-transparent border-0 cursor-pointer text-sm">
                <i className="bi bi-x-lg" />
              </button>
            )}
          </label>

          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4
            gap-2 overflow-y-auto pr-1">
            {filtered.length === 0 ? (
              <div className="col-span-full text-center py-10 text-base-content/40 text-sm">
                <i className="bi bi-search text-3xl block mb-2 text-base-content/20" />No products
              </div>
            ) : filtered.map((p, i) => {
              ...
              return (
                <motion.button key={p.id} variants={listItem} initial="hidden" animate="visible" custom={i}
                  ...
                  className={`relative text-left p-3 sm:p-3.5 rounded-xl border-2 transition-all
                  ...
                  {ci && (
                    <div className="absolute -top-2 -right-2 w-5 h-5 bg-primary text-white
                      text-[10px] font-bold rounded-full flex items-center justify-center shadow">
                      {ci.quantity}
                    </div>
                  )}
                  <div className="text-xs font-semibold text-base-content mb-1.5 leading-tight line-clamp-2">
                    {p.name}
                  </div>
                  ...
                  <div className={`text-[10px] mt-0.5
                    ${p.stockQty === 0 ? 'text-base-content/40' : p.isLowStock ? 'text-warning' : 'text-base-content/40'}`}>
        ...
        {/* Desktop cart */}
        <div className="hidden lg:flex w-[360px] xl:w-[380px] flex-col card bg-base-100
          border border-base-300 shadow-sm flex-shrink-0">
          <CartPanel />
        </div>
      </div>
      ...
      <AnimatePresence>
        {cartCount > 0 && (
          <motion.button
            ...
            className="lg:hidden fixed bottom-20 right-4 z-40 btn btn-primary rounded-2xl
              shadow-xl shadow-primary/30 gap-2 px-5">
            <i className="bi bi-cart3 text-base" />
            {cartCount} items · {fmt(finalAmount)}
          </motion.button>
        )}
      </AnimatePresence>
        ...
        {cartOpen && (
          <>
            ...
            <motion.div
              ...
              <div className="w-12 h-1 bg-base-300 rounded-full mx-auto mt-3 mb-1" />
      ...
      <AnimatePresence>
        {receipt && (
          <motion.div variants={scaleIn} initial="hidden" animate="visible" exit="exit"
            ...
            <div className="card bg-base-100 w-full sm:max-w-[420px] shadow-2xl overflow-hidden
              rounded-t-2xl sm:rounded-2xl"
              onClick={e => e.stopPropagation()}>
              <div className="bg-neutral text-white p-4 sm:p-5 text-center">
                ...
                <div className="flex justify-center gap-3 mt-2 text-xs text-white/50 font-mono flex-wrap">
              ...
              <div className="p-3 sm:p-4 max-h-[35vh] overflow-y-auto">
                <div className="grid grid-cols-[2fr_.5fr_1fr_1fr] text-[10px] font-bold uppercase
                  text-base-content/40 pb-2 mb-1 border-b border-base-300">
                  <span>Item</span><span>Qty</span><span>Price</span><span className="text-right">Total</span>
                </div>
                {receipt.items.map((item, i) => (
                  <div key={i} className="grid grid-cols-[2fr_.5fr_1fr_1fr] text-xs py-1.5
                    border-b border-dashed border-base-200 last:border-0">
              ...
              <div className="px-4 pb-2 flex flex-col gap-1">
                ...
                <div className="flex justify-between items-center font-bold text-base border-t-2 border-base-content pt-2 mt-1">
