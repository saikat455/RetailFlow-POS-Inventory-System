...
export default function POS() {
  ...
  // Load branches (Admin only — cashier already has fixed branchId)
  useEffect(() => {
    if (user?.isAdmin) {
  ...
  const addToCart = (product) => {
    ...
    setCart(prev => {
      ...
      return [...prev, {
        ...
        unitPrice: product.sellingPrice, maxQty: product.stockQty, quantity: 1
      }]
    })
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      setReceipt(res.data)
      // Refresh branch products to show updated stock
  ...
  if (!branchId) return (
    <div className="flex items-center justify-center h-64 text-gray-400 text-sm">
      <div className="text-center"><i className="bi bi-building text-4xl block mb-2 text-gray-200" />Select a branch to start selling</div>
    </div>
  )
    ...
    <div className="grid grid-cols-[1fr_380px] gap-5 h-[calc(100vh-64px)] max-h-[860px]">
      ...
      <div className="flex flex-col gap-3 overflow-hidden">
        ...
        {user?.isAdmin && branches.length > 0 && (
          <div className="flex items-center gap-2 bg-white rounded-xl border border-gray-200 px-3 py-2 flex-shrink-0">
            <i className="bi bi-building-fill text-blue-500 text-sm" />
            ...
            <select value={branchId} onChange={e => setBranchId(parseInt(e.target.value))}
              className="flex-1 text-sm font-semibold text-gray-800 border-0 outline-none bg-transparent cursor-pointer font-[inherit]">
              {branches.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}
            </select>
            <span className="text-xs text-gray-300 font-mono">{products.length} products</span>
          </div>
        )}

        {/* Branch badge (Cashier) */}
        {!user?.isAdmin && (
          <div className="flex items-center gap-2 bg-blue-50 border border-blue-100 rounded-xl px-3 py-2 flex-shrink-0">
            <i className="bi bi-building-fill text-blue-400 text-sm" />
            <span className="text-sm font-semibold text-blue-700">{user?.branchName}</span>
            <span className="ml-auto text-xs text-blue-400">{products.length} products</span>
          </div>
        )}
        ...
        <div className="relative flex-shrink-0">
          ...
          {search && (
            <button onClick={() => setSearch('')}
              className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 bg-transparent border-0 cursor-pointer text-sm">
        ...
        <div className="grid grid-cols-[repeat(auto-fill,minmax(135px,1fr))] gap-2.5 overflow-y-auto pr-1">
          {filtered.length === 0 ? (
            <div className="col-span-full text-center py-10 text-gray-400 text-sm">
              <i className="bi bi-search text-3xl block mb-2 text-gray-200" />No products found
            </div>
          ) : filtered.map(p => {
            ...
            return (
              <button key={p.id} onClick={() => addToCart(p)} disabled={p.stockQty === 0}
                ...
                <div className={`text-[10px] mt-1 ${p.stockQty === 0 ? 'text-gray-400' : p.isLowStock ? 'text-amber-500' : 'text-gray-400'}`}>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3.5 border-b border-gray-100 flex-shrink-0">
          ...
          {cart.length > 0 && (
            <button onClick={clearCart} className="text-red-400 hover:text-red-500 text-xs font-semibold bg-transparent border-0 cursor-pointer font-[inherit]">
        ...
        {cart.length === 0 ? (
          ...
          <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-2">
            {cart.map(item => (
              <div key={item.productId} className="flex items-center gap-2 bg-gray-50 rounded-xl px-3 py-2.5 border border-gray-100">
                <div className="flex-1 min-w-0">
                  <div className="text-xs font-semibold text-gray-800 truncate">{item.name}</div>
                  <div className="text-[11px] text-gray-400 font-mono">{fmt(item.unitPrice)}</div>
                </div>
                <div className="flex items-center gap-1 flex-shrink-0">
                  <button onClick={() => updateQty(item.productId, item.quantity - 1)}
                    className="w-6 h-6 flex items-center justify-center border border-gray-200 rounded-md bg-white text-gray-600 hover:bg-blue-500 hover:text-white hover:border-blue-500 transition-all text-xs cursor-pointer font-bold">−</button>
                ...
                <div className="text-xs font-bold font-mono text-blue-500 min-w-[52px] text-right">{fmt(item.unitPrice * item.quantity)}</div>
                <button onClick={() => removeFromCart(item.productId)} className="text-gray-300 hover:text-red-400 text-xs ml-0.5 bg-transparent border-0 cursor-pointer">
                  <i className="bi bi-x-lg" />
                </button>
              </div>
            ))}
        ...
        {/* Customer fields */}
        <div className="px-3 py-2 border-t border-gray-100 flex flex-col gap-2 flex-shrink-0">
          <div className="grid grid-cols-2 gap-2">
            <input value={customerName} onChange={e => setCustomerName(e.target.value)} placeholder="Customer name (opt.)"
              className="px-3 py-2 border border-gray-200 rounded-lg text-xs font-[inherit] outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 bg-gray-50" />
        ...
        <button onClick={handleCheckout} disabled={!cart.length || loading}
          className="mx-3 mb-3 flex items-center justify-center gap-2 bg-blue-500 hover:bg-blue-600 disabled:opacity-40
            text-white py-3.5 rounded-xl text-sm font-bold transition-all hover:-translate-y-0.5 hover:shadow-lg hover:shadow-blue-200
            cursor-pointer border-0 font-[inherit] disabled:cursor-not-allowed">
          {loading
      ...
      {receipt && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[1000] p-5"
          ...
          <div className="bg-white rounded-2xl w-full max-w-[420px] shadow-2xl overflow-hidden"
            style={{ animation: 'popIn 0.2s ease' }} onClick={e => e.stopPropagation()}>
            <div className="bg-[#13151f] text-white p-5 text-center">
              ...
              <div className="flex justify-center gap-3 mt-2 text-xs text-white/50 font-mono">
                <span className="font-bold text-blue-400">{receipt.invoiceNo}</span>
            ...
            <div className="p-4">
              <div className="grid grid-cols-[2fr_.5fr_1fr_1fr] text-[10px] font-bold uppercase text-gray-400 pb-2 mb-1 border-b border-gray-100">
                <span>Item</span><span>Qty</span><span>Price</span><span className="text-right">Total</span>
              </div>
              {receipt.items.map((item, i) => (
                <div key={i} className="grid grid-cols-[2fr_.5fr_1fr_1fr] text-xs py-1.5 border-b border-dashed border-gray-100 last:border-0">
            ...
            <div className="mx-4 mb-3 bg-green-50 border border-green-100 rounded-lg px-3 py-2 text-xs text-green-600 font-semibold flex items-center gap-2">
